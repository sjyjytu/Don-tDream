...
bool BuildTexture(TCHAR* szPathName, GLuint& texid)      // 载入图片并转换为纹理  
{
	...
	if (_tcsstr(szPathName, L"http://"))         // 如果路径包含 http:// 则...  
	{
		_tcscpy(szPath, szPathName);          // 把路径拷贝到 szPath  
	}
	else                // 否则从文件导入图片  
	{
		GetCurrentDirectory(MAX_PATH, szPath);       // 取得当前路径  
		_tcscat(szPath, L"\\");           // 添加字符"\"  
		_tcscat(szPath, szPathName);          // 添加图片的相对路径  
	}
	...
	{
		// 图片载入失败出错信息  
		MessageBox(HWND_DESKTOP, L"图片导入失败!\n(TextureLoad Failed!)", L"Error", MB_OK | MB_ICONEXCLAMATION);
...
bool BuildTexture(TCHAR* filename, TextureTga* texture)     // 载入一个.TGA 文件到内存  
{
	...
	FILE* file = _tfopen(filename, L"rb");         // 打开 TGA 文件  
